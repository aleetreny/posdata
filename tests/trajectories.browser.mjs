...
try {
 ...
 await go('?view=trajectories&trOrigin=all&trEmployer=Faculdade+de+Ci%C3%AAncias+da+Universidade+de+Lisboa');
 assert.ok(await page.locator('.trajectory-record').count()>0);
 for(const employer of await page.locator('.trajectory-destination > strong').all()) assert.equal(await employer.innerText(),'Faculdade de Ciências da Universidade de Lisboa');
 for(const sector of await page.locator('.trajectory-destination > small').all()) assert.equal(await sector.innerText(),'Universidades y educación');
 await page.locator('.trajectory-detail-toggle').first().click();await page.locator('.career-timeline').first().waitFor();
 assert.match(await page.locator('.career-evidence').innerText(),/unidad de la universidad indicada/);
 await page.getByRole('button',{name:'Quitar filtro: Faculdade de Ciências da Universidade de Lisboa',exact:true}).click();await ready();
 pass('Lisbon faculties classify as education, show parent evidence and support removable exact-employer filters');
 await go('?view=trajectories&trOrigin=all&trQ=Oxford&trSearchIn=employer');
 for(const employer of await page.locator('.trajectory-destination > strong').all()) assert.match(await employer.innerText(),/Oxford/i);
 await page.getByRole('combobox',{name:'Buscar en',exact:true}).selectOption('doctoral');await ready();
 for(const origin of await page.locator('.trajectory-origin > p').all()) assert.match(await origin.innerText(),/Oxford/i);
 pass('employer and doctoral-university searches have independent semantics');
 await page.goto(base+'?view=careers&lang=es');await page.getByRole('searchbox',{name:'Buscar funciones',exact:true}).fill('datos');
 await page.getByRole('link',{name:'Ver puestos de datos y software',exact:true}).click();await ready();
 assert.match(page.url(),/trRole=3/);assert.ok(await page.locator('.trajectory-record').count()>0);
 assert.ok(await page.getByRole('button',{name:'Quitar filtro: Datos y software',exact:true}).isVisible());
 pass('career option leads directly to real examples with a visible function filter');
